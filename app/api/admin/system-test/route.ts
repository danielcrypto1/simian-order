...
async function runTest(
  name: string,
  fn: () => Promise<{ ok: boolean; message: string; cleanup?: () => Promise<void> }>
): Promise<TestResult> {
  const t0 = Date.now();
  ...
  try {
    ...
    if (cleanup) {
      try { await cleanup(); } catch { /* swallow cleanup errors */ }
    }
  }
}
...
export async function POST(req: Request) {
  ...
  // 1. Application Flow ──────────────────────────────────────────────
  tests.push(
    await runTest("Application Flow", async () => {
      ...
      if (app.status !== "pending") {
        return {
          ok: false,
          message: `expected status=pending after submit, got ${app.status}`,
          cleanup: async () => { await deleteApplication(wallet); },
        };
      }
      ...
      if (!found) {
        return {
          ok: false,
          message: "submitted application not visible in admin list",
          cleanup: async () => { await deleteApplication(wallet); },
        };
      }
      if (found.twitter !== handle) {
        return {
          ok: false,
          message: `twitter mismatch: stored=${found.twitter} sent=${handle}`,
          cleanup: async () => { await deleteApplication(wallet); },
        };
      }
      return {
  ...
  tests.push(
    await runTest("Approval Flow", async () => {
      ...
      // The frontend gates referral on application_status === "approved" via
      // RouteGuard; status flip is the unlock signal.
      return {
        ok: true,
  ...
  // 3. Referral Flow ─────────────────────────────────────────────────
  tests.push(
    await runTest("Referral Flow", async () => {
      ...
      if (!r.ok) {
        return { ok: false, message: `addReferral failed: ${r.error}`, cleanup };
      }
      ...
      if (dup.ok) {
        return { ok: false, message: "duplicate referee should have been rejected", cleanup };
      }
  ...
  // 4. FCFS Flow ─────────────────────────────────────────────────────
  // Uses HTTP to exercise the route handler + middleware (public).
  tests.push(
    await runTest("FCFS Flow", async () => {
      ...
      if (claim.taken !== before.taken + 1) {
        return {
          ok: false,
          message: `taken did not increment: ${before.taken} → ${claim.taken}`,
        };
      }
      // Double-claim must 409.
    ...
    await runTest("Signature", async () => {
      ...
      try {
        ...
        return {
          ...
          message:
            e instanceof Error
              ? `generateSignature threw: ${e.message}`
              : "generateSignature threw",
        };
      }
      if (typeof signature !== "string" || !signature.startsWith("0x") || signature.length !== 132) {
        return {
          ok: false,
          message: `bad signature shape (len=${signature.length}): ${signature.slice(0, 20)}…`,
        };
      }
      ...
      if (recovered.toLowerCase() !== announced.toLowerCase()) {
        return {
          ok: false,
          message: `recovered ${recovered} ≠ signer ${announced}`,
        };
      }
      return {
        ok: true,
  ...
  return NextResponse.json({ tests, total: tests.length, passed, failed });
}
